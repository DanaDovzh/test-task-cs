...
export class FormPersonComponent implements OnInit {
  ...
  mode: ModeDialog;
  numberCarMask: RegExp[] = [/[A-Z]/, /[A-Z]/, /[0-9]/, /[0-9]/, /[0-9]/, /[0-9]/, /[A-Z]/,  /[A-Z]/];
  patternForValidator: RegExp = /^[A-Z]{2}[0-9]{4}[A-Z]{2}$/;
  carForm: FormGroup =  this.fb.group({
    year: ['', [Validators.required, Validators.min(1990), Validators.max(this.thisYear)]],
    brand: ['', Validators.required],
    model: ['', Validators.required],
    number: ['', [Validators.required, Validators.pattern(this.patternForValidator)]],
  });
  ...
  initForm() {
    if(this.data.mode !== 'create') {
      ...
      this.data.carInfo.forEach(car => {
        (this.formUserData.get('carInfo') as FormArray).push(
          this.fb.group({
            year: [car.year, [Validators.required, Validators.min(1990), Validators.max((new Date().getFullYear()))]],
            brand: [car.brand, Validators.required],
            model: [car.model, Validators.required],
            number: [car.number, [Validators.required, Validators.pattern(this.patternForValidator)]],
          })
        )
      })
      ...
      this.formUserData = this.fb.group({
        ...
        carInfo: this.fb.array([_.cloneDeep(this.carForm)])
      });
    }
    ...
    this.formUserData.value.carInfo.forEach(car => {
      if(car.number) {
        nowCars.push(car.number)
      }
    })
      this.carsNumber = [...this.data.carsNumber.map(number => {
        if (!nowCars.includes(number)) {
          return number;
        }
  ...
  checkUniqNumber() {
    ...
    let all = this.carsNumber.concat(nowCars);
    let uniNumber = [...new Set(all)];
    return all.length === uniNumber.length
  }
